...
fn setup_db(path: &Path) -> Result<rusqlite::Connection, Box<dyn std::error::Error>> {
    ...
    conn.execute_batch(include_str!("../../src/db/schemas/passwd.sql"))?;
    conn.execute_batch(include_str!("../../src/db/schemas/groups.sql"))?;
    conn.execute_batch(include_str!("../../src/db/schemas/shadow.sql"))?;

    env::set_var("NSS_PASSWD_PATH", path);
    env::set_var("NSS_GROUP_PATH", path);
    env::set_var("NSS_SHADOW_PATH", path);
