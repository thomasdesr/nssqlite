...
fn get_existing_group() -> common::TestResult<()> {
    ...
    let group_by_name =
        uzers::get_group_by_name("test-group").expect("failed to find group by name");
    let group_by_id = uzers::get_group_by_gid(321).expect("failed to find group by id");
...
fn get_missing_group() -> common::TestResult<()> {
    ...
    let user_by_name = uzers::get_group_by_name("missing-group");
    let user_by_id = uzers::get_group_by_gid(999);
